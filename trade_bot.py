...
# Now includes real-time alerts for trigger states and buy signals.
#
# To Run:
# 1. Ensure 'collector.py' and 'app.py' are running.
# 2. Configure your trade size in the 'TRADE_USD_SIZE' variable below.
# 3. Set up your Discord webhook URL (optional but recommended)
# 4. Run this script from your terminal: python trade.py
...
def check_alert_conditions(trigger_data):
    ...
    if current_trigger_armed and last_alert_states['trigger_armed'] != True:
    ...
    elif not current_trigger_armed and last_alert_states['trigger_armed'] != False:
        reset_threshold = fib_0_level * 1.005 if fib_0_level else 0
        message = f"🔴 TRIGGER DISARMED! Price: ${current_price:.2f} above reset threshold: ${reset_threshold:.2f}"
        send_alert(message, "info")
        last_alert_states['trigger_armed'] = False
    
    # Alert for BUY SIGNAL (only once per signal)
    if current_buy_signal and last_alert_states['buy_signal'] != True:
    ...
    elif not current_buy_signal and last_alert_states['buy_signal'] != False:
...
def main():
    ...
    while True:
        try:
            ...
            if trigger_data:
                # Check for alert conditions first
                ...
                if trigger_data.get('buy_signal'):
                    ...
                    if order_result.get("status") == "ok":
                        message = f"✅ TRADE EXECUTED! Bought {order_size_in_asset} SOL at ${market_price:.2f} | Total: ${TRADE_USD_SIZE}"
            ...
            error_message = f"❌ Bot error occurred: {str(e)}"
