# Hyperliquid All-in-One Trading & Monitoring Bot
...
# <-- MODIFIED: Function now accepts the 'address' string
def get_open_positions(address, info, trade_log):
    """
    Fetches open positions and matches them with the trade log.
    Returns a list of formatted position dictionaries.
    """
    try:
        # <-- MODIFIED: Use the passed-in address directly
        user_state = info.user_state(address)
    except Exception as e:
        print(f"[!] Could not fetch user state: {e}")
        return []
    ...
    for position in open_positions:
        ...
        processed_positions.append({
            ...
            "pnl": float(pos_info['unrealizedPnl']),
...
def send_discord_alert(message, color=0x00ff00, savant_data=None, open_positions=None):
    ...
    try:
        embed = {
            "title": f"🤖 {TRADE_ASSET} Trading Bot Alert",
            "description": message,
            "color": color,
            "timestamp": datetime.now().isoformat(),
            "footer": {"text": "Hyperliquid All-in-One Bot"}
        }
        fields = []
        if savant_data:
            fields.extend([
        ...
        if open_positions:
            for pos in open_positions:
                ...
                field_value = (
                    f"**`{pos['direction']} {pos['size']:.4f} {pos['asset']}`**\n"
...
# <-- MODIFIED: Function now accepts 'address' and 'info'
def send_status_update(address, info):
    """Fetches position status and sends a 15-minute update."""
    print(f"[*] Sending {STATUS_UPDATE_MINUTES}-minute status update...")
    trade_log = load_trade_log()
    # <-- MODIFIED: Pass the address to get_open_positions
...
# (The rest of your functions like read_last_savant_record, log_trade, etc. remain here, unchanged)
def read_last_savant_record():
    ...
    try:
        with open(PRICE_SAVANT_FILE, 'rb') as f:
            ...
            # Find the last occurrence of '{' to locate the start of the last JSON object
            last_obj_start = buffer.rfind('{')
            if last_obj_start == -1: return None
            
            # Attempt to parse from this point forward
...
def main():
    ...
    try:
        ...
        # <-- MODIFIED: Schedule now passes both 'address' and 'info'
        schedule.every(STATUS_UPDATE_MINUTES).minutes.do(send_status_update, address=address, info=info)
        
        send_discord_alert(f"🤖 **Bot Started.** Monitoring **{TRADE_ASSET}**.", 0x888888)
        print(f"\n[*] Bot running. Status updates every {STATUS_UPDATE_MINUTES} mins.")
        print("-" * 50)

        # <-- MODIFIED: Initial status update call is also fixed
        ...
        while True:
            ...
            if latest_record and latest_record.get('buy_signal') and latest_record.get('timestamp') != last_traded_signal_timestamp:
                ...
                send_discord_alert(f"🚀 **BUY SIGNAL!** Preparing trade...", 0x00ff00, savant_data=latest_record)
                ...
                print(f"[*] Placing market buy for {order_size_in_asset} {coin}...")
